...
import { Edit2, Plus, Trash2, Upload, X, Briefcase, GraduationCap, Link2, GitBranch, Globe, FileText, Download, Eye, CheckCircle } from 'lucide-react';
...
export default function CandidateProfile() {
  ...
  if (loading) return <div className="animate-pulse space-y-4">{[...Array(4)].map((_, i) => <div key={i} className="h-32 bg-gray-200 dark:bg-gray-800 rounded-xl" />)}</div>;
    ...
    <div className="max-w-3xl mx-auto space-y-5">
      ...
      <Card className="p-5">
        ...
        {profile?.resumeUrl ? (
          <div className="border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden">
            ...
            <div className="flex items-center gap-2 p-3 border-t border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-900">
              ...
              <a
                ...
                className="flex-1 flex items-center justify-center gap-2 px-3 py-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-200 text-sm font-medium rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors"
              >
                <Download size={14} /> Download
              </a>
              <label className="flex items-center justify-center gap-2 px-3 py-2 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-200 text-sm font-medium rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors cursor-pointer">
                <Upload size={14} />
              ...
              <button
                ...
                className="flex items-center justify-center gap-1 px-3 py-2 bg-white dark:bg-gray-800 border border-red-200 dark:border-red-800 text-red-500 text-sm font-medium rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors"
                title="Remove resume"
              >
                <Trash2 size={14} />
              </button>
          ...
          /* Upload Area */
          <label className="block border-2 border-dashed border-gray-300 dark:border-gray-600 rounded-xl p-8 text-center cursor-pointer hover:border-primary-400 hover:bg-primary-50/30 dark:hover:bg-primary-900/10 transition-all group">
            <input
              type="file"
              className="hidden"
              accept=".pdf,.doc,.docx"
              ...
              onChange={e => handleResumeUpload(e.target.files[0])}
            />
            <div className="w-14 h-14 rounded-2xl bg-gray-100 dark:bg-gray-800 group-hover:bg-primary-100 dark:group-hover:bg-primary-900/30 flex items-center justify-center mx-auto mb-3 transition-colors">
              {uploading
      ...
      <Card className="p-5">
        ...
        <div className="flex flex-wrap gap-2 mb-3">
          {(profile?.skills || []).map(s => (
            <Badge key={s} className="bg-primary-100 text-primary-700 dark:bg-primary-900/30 dark:text-primary-400 gap-1">
              {s}
            </Badge>
          ))}
              ...
              <div className="flex-1">
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex gap-1">
                    <button onClick={() => { setEditingExp(exp); setExpForm(exp); setExpModal(true); }} className="p-1.5 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg"><Edit2 size={14} className="text-gray-400" /></button>
                    <button onClick={() => deleteExp(exp.id)} className="p-1.5 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg"><Trash2 size={14} className="text-red-400" /></button>
                  </div>
                </div>
            ...
            <div key={edu.id} className="flex gap-3">
              ...
              <div className="flex-1">
                <div className="flex items-start justify-between">
                  ...
                  <div className="flex gap-1">
                    <button onClick={() => { setEditingEdu(edu); setEduForm(edu); setEduModal(true); }} className="p-1.5 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg"><Edit2 size={14} className="text-gray-400" /></button>
                    <button onClick={() => deleteEdu(edu.id)} className="p-1.5 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg"><Trash2 size={14} className="text-red-400" /></button>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2 mb-2">
              {(profileForm.skills || []).map(s => (
                <Badge key={s} className="bg-primary-100 text-primary-700 dark:bg-primary-900/30 dark:text-primary-400 gap-1 cursor-pointer" onClick={() => removeSkill(s)}>
                  {s} <X size={12} />
                </Badge>
              ))}
            </div>
