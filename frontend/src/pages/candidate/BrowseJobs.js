...
import { Search, MapPin, Filter, BookmarkPlus, BookmarkCheck, Briefcase, Building2, DollarSign, Clock, X, Upload } from 'lucide-react';
...
import { STATUS_CONFIG, JOB_TYPE_CONFIG, formatSalary, formatRelativeDate, cn, getFileUrl } from '../../lib/utils';
...
export default function BrowseJobs() {
 ...
 className={cn('px-3 py-1.5 rounded-full text-sm font-medium border transition-all', filters.type === type ? 'bg-primary-600 text-white border-primary-600' : 'border-gray-200 text-gray-600 hover:border-gray-300')}
 >
 {cfg?.label || type}
 </button>
 );
 ...
 <Link to={`/candidate/company/${job.companyId || job.company?.id}`} className="text-sm text-gray-500 hover:text-primary-600 flex items-center gap-1 mt-0.5 inline-flex" onClick={e => e.stopPropagation()}>
 <Building2 size={13} /> {job.company?.name}
 </Link>
 </div>
 <button
 onClick={e => toggleSave(job.id, e)}
 className="p-1.5 rounded-lg hover:bg-gray-100 :bg-gray-800 transition-colors shrink-0"
 >
 {isSaved
 ...
 {job.skills?.slice(0, 2).map(s => <Badge key={s} className="bg-gray-100 text-gray-600 ">{s}</Badge>)}
 </div>
 ...
 {selected.company?.logo ? <img src={getFileUrl(selected.company.logo)} alt="" className="w-full h-full object-contain rounded-xl" /> : <Briefcase size={20} className="text-gray-400" />}
 </div>
 <div>
 <Link to={`/candidate/company/${selected.companyId || selected.company?.id}`} className="font-semibold text-lg text-gray-900 hover:text-primary-600 hover:underline inline-block">{selected.company?.name}</Link>
 <div className="flex items-center gap-3 text-sm text-gray-500 mt-1">
 <span className="flex items-center gap-1"><MapPin size={13} /> {selected.company?.location || selected.location}</span>
 {selected.company?.industry && <span className="flex items-center gap-1">🏢 {selected.company.industry}</span>}
 </div>
 <div className="flex items-center gap-4 text-xs text-gray-500 mt-1">
 {selected.company?.user?.email && <span>✉️ {selected.company.user.email}</span>}
 {selected.company?.user?.phone && <span>📞 {selected.company.user.phone}</span>}
 </div>
 </div>
 </div>
 ...
 <p className="text-sm text-gray-600 whitespace-pre-wrap leading-relaxed">{selected.requirements}</p>
 </div>
 )}
 ...
 <div className="flex flex-wrap gap-2">
 {selected.skills.map(s => <Badge key={s} className="bg-primary-100 text-primary-700 ">{s}</Badge>)}
 </div>
 </div>
 )}
 ...
 {selected.status === 'ACTIVE' ? (
   <Button onClick={() => { setApplyModal(true); }} className="flex-1 bg-[#635BFF] hover:bg-[#5146E5] text-white">Apply Now</Button>
 ) : (
   <div className="flex-1 p-2 bg-red-50 text-red-600 rounded-lg text-center font-medium border border-red-200">
     Applications Closed
   </div>
 )}
 ...
 {user?.candidate?.resumeName && !resumeFile && (
 <p className="text-xs text-gray-500 mt-2 flex items-center gap-1">
 <BookmarkCheck size={12} className="text-green-500" />
 Current resume: {user.candidate.resumeName}
 </p>
 )}
 {!user?.candidate?.resumeName && !resumeFile && (
 <p className="text-xs text-red-500 mt-1">Please upload a resume</p>
 )}
 </div>
