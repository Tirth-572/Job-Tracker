...
import { STATUS_CONFIG, formatRelativeDate, getGreeting } from '../../lib/utils';
...
export default function CompanyDashboard() {
 ...
 <div>
 <h1 className="text-2xl font-bold text-gray-900 ">{getGreeting()}, {user?.company?.name || 'Company'}! 👋</h1>
 ...
 <StatCard icon={Briefcase} label="Total Jobs Posted" value={stats?.totalJobs || 0} color="primary" />
 <StatCard icon={Eye} label="Active Jobs" value={stats?.activeJobs || 0} color="green" />
 <StatCard icon={CheckCircle} label="Closed Jobs" value={stats?.closedJobs || 0} color="primary" />
 <StatCard icon={TrendingUp} label="Expired Jobs" value={stats?.expiredJobs || 0} color="amber" />
 </motion.div>
 ...
 <Card className="p-5">
 <h2 className="font-semibold mb-4 text-gray-900 ">Application Pipeline</h2>
 {loading ? (
 ...
 <Card className="p-5">
 <h2 className="font-semibold mb-4 text-gray-900 ">Status Breakdown</h2>
 {loading ? (
 <div className="h-48 skeleton rounded-xl" />
 ) : pieData.length > 0 ? (
 <ResponsiveContainer width="100%" height={200}>
 ...
 return (
 <Link key={app.id} to={`/company/applications/${app.id}`} className="flex items-center gap-3 px-5 py-4 hover:bg-gray-50 :bg-gray-800/50 transition-colors">
 <div className="w-9 h-9 rounded-xl bg-primary-100 flex items-center justify-center text-primary-700 font-semibold text-sm shrink-0">
